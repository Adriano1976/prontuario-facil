...
- 🟡 **Remoção das 14 dependências declaradas e não utilizadas** (`@stripe/react-stripe-js`, `@stripe/stripe-js`, `react-leaflet`, `jspdf`, `html2canvas`, `lodash`, `react-quill`, `three`, `react-markdown`, `canvas-confetti`, `@hello-pangea/dnd`, `@radix-ui/react-toast`, `zod`, `@hookform/resolvers`). Não pertence a esta feature. **A reconfirmação por busca foi cumprida em 2026-09-18:** a varredura por nome de pacote em todo o `src/` — incluindo `src/components/ui/*.jsx`, que a verificação de tipos ignora e o empacotador não — e nos arquivos de configuração da raiz encontrou **zero** import dos 14 pacotes; o mesmo método aplicado a pacotes comprovadamente em uso devolveu 64 ocorrências, o que afasta falso negativo. Três ressalvas da conferência: `@radix-ui/react-toast` é dispensável **apesar** de o sistema de toast estar em uso (a implementação local monta a interface com `div`, sem Radix); `three` aparece apenas no mini-site `_reversa_docs/`, por arquivo vendorizado baixado de CDN conforme `vendor-pins.yaml`, nunca pelo `node_modules`; e `react-quill@0.0.2` é pacote placeholder que arrasta `quilljs@0.18.1` → `lodash@2.4.2`. **Falta a aprovação explícita**, e a remoção exige dono e escopo próprios: RF-13 segue `Won't` e D-09 mantém a remoção fora desta feature (RISK-006). Nenhuma dependência foi removida e o `package.json` não foi tocado.
