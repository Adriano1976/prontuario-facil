...
| D-06 | A correção do F-05 **não** tem **arquivo** de watch próprio — a guarda dela vive como `W013` no watch consolidado desta feature | Não há contrato de comportamento a preservar; a guarda é a prova, que alimenta o componente com uma cor hostil e afirma que nada executável nasce. Um arquivo de watch só para ela seria ruído, mas o item de vigilância **existe**, e é isso que o texto precisa dizer | a) criar um arquivo de watch de um item só — ruído sem função | 🟢 |
