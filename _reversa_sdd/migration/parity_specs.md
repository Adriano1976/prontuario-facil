...
hash: "sha256:da97588a115867b8c52ad4b91595108303d670f9ceae439fbcf2ed90682e0c48"
---
...
- **Modos de validação aplicáveis** (marcar os usados):
  ...
  - [x] Outro: **type-level gate** — `tsc --noEmit` = 0 como métrica primária (brief); **paridade visual por construção** em modo literal (mesma plataforma) + golden manual quando capturado.

## Critérios de "paridade aceita"
- **Métrica primária**: `tsc --noEmit` retorna **0 erros** com `strict: true` e 100% dos arquivos em `.ts`/`.tsx` (gate de aceite do brief); **zero** divergência funcional detectada nos characterization tests de fluxos críticos (45 BR-MIGRAR reproduzidos; nenhuma correção comportamental aplicada — AMB-001…006).
...
- **Dimensão adicional própria desta migração (tipos)**: paridade não é só funcional — é também **de tipos**: campos LGPD obrigatórios, unions de status/estado, escopo `created_by_id`/`role` exigido por tipos (F-01/F-03 detectáveis em compile-time). Um componente que compilava com `any` e mudava de comportamento ao ser tipado é divergência.
...
- **Carga / performance**: métricas do brief — bundle < 185 KB e build < 4 s (`tsc`/Vite); sem teste de carga de servidor (SPA, BaaS).
...
- **Exceções (deviations propagadas)**: `screen_deviation_log.md` registra **0 deviations** — nenhuma exceção a propagar. (AMB-006 XSS e F-01/F-02/F-03 são alertas referidos à codificação, não exceções de paridade visual.)
