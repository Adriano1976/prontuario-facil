# -*- coding: utf-8 -*-
"""Regera o actions.md numerando os IDs na ORDEM DE EXECUCAO.

Motivo: a numeracao anterior colocava a Fase 2 (Testes) antes do nucleo que ela
verifica, gerando 10 dependencias com referencia para frente (T007 -> T027 etc.).
IDs precisam ser estaveis e executaveis em ordem crescente, entao o documento e
gerado pela primeira vez aqui com a numeracao correta: nucleo -> testes ->
integracao -> polimento. O agrupamento por fase do template e preservado.

Escrita atomica. Gerador descartavel.
"""
import os
...
| Total de ações | 44 |
| Paralelizáveis (`[//]`) | 15 |
| Maior cadeia de dependência | 9 elos |

> **Estado inicial:** as ações `T001` a `T017` e `T020` a `T022` já foram executadas e
> verificadas antes da abertura desta feature. Entram com status `[X]` porque o trabalho
> existe no código — marcá-las como pendentes faria o `/reversa-coding` refazer o que já
> passou por verificação.
>
> **Ordem dos IDs:** a numeração segue a **ordem de execução**, não a ordem das seções
> do template. O núcleo vem antes dos testes porque um teste de verificação negativa só
> faz sentido depois que o contrato que ele verifica existe. Nenhuma ação depende de um
> ID maior que o próprio.
>
...
| ID | Descrição | Dependências | Paralelismo | Arquivo alvo | Confidência | Status |
|----|-----------|--------------|-------------|--------------|-------------|--------|
| T031 | Verificar por caso negativo que consentimento aceito sem data ou sem endereço de rede não compila | T008 | `[//]` | arquivo de verificação negativa | 🟢 | `[X]` |
| T032 | Verificar por caso negativo que valor fora dos conjuntos fechados de status e tipo não compila | T009, T010 | `[//]` | arquivo de verificação negativa | 🟢 | `[X]` |
| T033 | Verificar por caso negativo que ler dado clínico sem declarar escopo não compila | T019 | - | arquivo de verificação negativa | 🟢 | `[X]` |
| T034 | Verificar por caso negativo que informar o dono manualmente num filtro já escopado não compila | T019 | - | arquivo de verificação negativa | 🟢 | `[X]` |
| T035 | Confirmar por uso correto que a leitura com escopo compila e aplica o filtro de dono | T019 | - | arquivo de verificação negativa | 🟢 | `[X]` |
| T036 | Verificar por caso negativo que nome de entidade inexistente não compila | T020 | `[//]` | arquivo de verificação negativa | 🟢 | `[X]` |
