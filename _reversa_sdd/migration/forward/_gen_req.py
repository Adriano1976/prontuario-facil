# -*- coding: utf-8 -*-
"""Gerador do requirements.md da feature 001-migracao-typescript.

Executado uma vez por /reversa-requirements. Faz escrita atomica (tmp + rename)
conforme a secao "Persistencia" do skill. Este gerador e descartavel.
"""
import json
import os
...
FEATURE_DIR = os.path.join(ROOT, "_reversa_forward", "001-migracao-typescript")
ACTIVE = os.path.join(ROOT, ".reversa", "active-requirements.json")
...
|------|-----------|-------|
| DATA_AQUI | Versão inicial gerada por `/reversa-requirements` | reversa |
| DATA_AQUI | Registrado o ponto de partida correto (1.324 erros) e os limites explícitos de alcance da verificação de tipos | reversa |
"""
...
def main():
    ...
    target = os.path.join(FEATURE_DIR, "requirements.md")
    tmp = target + ".tmp"
    with open(tmp, "w", encoding="utf-8", newline="\n") as fh:
        fh.write(BODY.replace("DATA_AQUI", hoje).lstrip("\n"))
    os.replace(tmp, target)
    print("requirements.md escrito:", target)

    active = {
        "schema-version": 1,
        "feature-dir": "_reversa_forward/001-migracao-typescript",
        "feature-id": "001",
        "short-name": "migracao-typescript",
        "started-at": agora,
    ...
    print("active-requirements.json escrito:", ACTIVE)
    print("marcadores de duvida:", BODY.count("[DÚVIDA]"))
