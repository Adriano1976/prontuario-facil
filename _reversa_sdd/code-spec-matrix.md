...
> **O bloqueio declarado estava vencido — e a lacuna que o sustentava fechou depois.** Esta matriz
> registrava o grupo `08` como dependente da lacuna `G-01`. A dependência era de **produto**, não de
> prova: `PT-008.4` congelava o valor por decisão humana (`AMB-001`), e `Dashboard.tsx:173` já
> entregava `"94%"`. Em 2026-09-25 a feature `016-taxa-de-atendimento` **fechou `G-01`** — fórmula,
> fonte e período decididos e implementados —, e `PT-008.4` mudou de propósito por decisão, como o
> `O004` do watch da feature `009` previa.
...
| PT-008.4 — Taxa de Atendimento (cenário superado por decisão) | `parity_tests/08-kpis-dashboard.feature` | 🟢 **provado, cenário superado** | `DashboardKpis.test.tsx` — o cartão exibe o **travessão com o texto do estado sem base** quando a massa não tem desfecho, e nenhuma tendência é renderizada. **O cenário foi superado por decisão em 2026-09-25**: ele enunciava a constante `"94%"`, que a feature `016-taxa-de-atendimento` substituiu pelo cálculo, fechando `G-01`. A metade do cenário que dizia "vindo de **constante tipada**" era **falsa** e deixou de ser irrelevante: agora existe símbolo real (`src/lib/taxaAtendimento.ts`), ainda que não com o nome `TAXA_ATENDIMENTO_MOCK` — a decisão de 2026-09-25 definiu a fórmula em vez de congelar o valor. A prova do cálculo e da janela vive em `src/lib/__tests__/taxaAtendimento.test.ts` |
...
| **A constante decidida de `AMB-001` nunca existiu** | ✅ **Resolvida** em 2026-09-25 pela feature `016-taxa-de-atendimento`. A decisão humana de 2026-09-09 exigia "manter `94%` como **constante explícita e tipada** (`TAXA_ATENDIMENTO_MOCK = 94`)" e o código tinha o literal `value="94%"` em `Dashboard.tsx:173`. **O literal deixou de existir**: a fórmula vive em `src/lib/taxaAtendimento.ts`, com constantes nomeadas, função pura e prova própria. A divergência entre o decidido e o implementado está encerrada. O símbolo não se chama `TAXA_ATENDIMENTO_MOCK` porque a decisão de 2026-09-25 foi mais longe: definiu a fórmula em vez de congelar o valor |
...
| **A Taxa de Atendimento está resolvida num artefato e pendente noutro** | ✅ **Resolvida** em 2026-09-25 pela feature `016-taxa-de-atendimento`. A divergência era entre `dashboard/requirements.md:17` (🔴) e `migration/target_domain_model.md:88` (`AMB-001 resolvido`). A lacuna de **produto** foi fechada: fórmula, fonte e período foram decididos na sessão de esclarecimentos (`_reversa_forward/016-taxa-de-atendimento/requirements.md#9`) e a regra do Dashboard deixou de ser 🔴 |
