...
import type { Doctor, Weekday, WorkingHours } from '@/types';

const DAYS_OF_WEEK: { value: Weekday; label: string }[] = [
...
const estadoInicial = (): DoctorFormState => ({
    full_name: '',
    specialty: '',
    crm: '',
    email: '',
...
export default function Doctors() {
    ...
    const [formData, setFormData] = useState<DoctorFormState>(estadoInicial);
    ...
    const saveMutation = useMutation({
        mutationFn: async (data: DoctorFormState) => {
            if (editingDoctor) {
    ...
    const deleteMutation = useMutation({
        mutationFn: (id: string) => base44.entities.Doctor.delete(id),
    ...
    const handleEdit = (doctor: Doctor) => {
        ...
        // O registro entra no formulário como veio: campos ausentes no registro
        // (ex.: janela de atendimento) ficam ausentes no formulário, como no legado.
        setFormData(doctor);
        setShowEditor(true);
    };
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setFormData({ ...formData, working_hours: { ...(formData.working_hours ?? {}), start: e.target.value } as WorkingHours })}
                                />
                            </div>
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setFormData({ ...formData, working_hours: { ...(formData.working_hours ?? {}), end: e.target.value } as WorkingHours })}
                                />
                            </div>
                            <div className="space-y-2">
