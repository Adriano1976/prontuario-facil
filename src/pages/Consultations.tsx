 ...
 * busca por paciente/queixa/diagnóstico, os filtros de status e de intervalo de
 * data, a legenda de situação (incluindo a ausência de legenda quando o registro
 * não tem status), e o estado vazio.
 *
 * PARIDADE DE LEITURA: as duas consultas passam a declarar o escopo de acesso pela
 * decisão de escopo da feature (opção C) — a mesma condição que a RLS do servidor
 * já aplicava.
 */
export default function Consultations() {
    ...
    const { data: consultations, isLoading: loadingConsultations } = useQuery({
        ...
        queryFn: async () => {
            ...
            return base44.entities.Consultation.listOwned(resolveScope(user), '-date');
        },
    });
    ...
    const statusInfo = (consultation: Consultation) =>
        consultation.status ? STATUS_CONFIG[consultation.status] : null;
                ...
                {loadingConsultations ? (
                    ...
                    <div className="grid gap-4">
                        {filteredConsultations.map((consultation, index) => {
                            ...
                            const status = statusInfo(consultation);
                            return (
                                <motion.div
                                    key={consultation.id}
                                    initial={{ opacity: 0, y: 20 }}
