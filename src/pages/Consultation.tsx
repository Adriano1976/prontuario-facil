...
import type { ConsultationStatus, Exam, Prescription } from '@/types';
 ...
 * quatro consultas (consulta, paciente, prescrições e exames), o registro de acesso,
 * a legenda de situação (incluindo a ausência de legenda quando o registro não tem
 * status), a impressão e os diálogos de documento e exame.
 *
...
export default function Consultation() {
    ...
    const { data: consultationData, isLoading } = useQuery({
        ...
        queryFn: () => readOwned(base44.entities.Consultation, { id: consultationId ?? '' }),
    ...
    const { data: patientData } = useQuery({
        ...
        queryFn: () => readOwned(base44.entities.Patient, { id: consultation?.patient_id ?? '' }),
    ...
    const { data: prescriptions } = useQuery({
        ...
        queryFn: () => readOwned(base44.entities.Prescription, { consultation_id: consultationId ?? '' }, '-created_date'),
        enabled: !!consultationId,
    });

    const { data: exams } = useQuery({
        queryKey: ['exams', consultationId],
        queryFn: () => readOwned(base44.entities.Exam, { consultation_id: consultationId ?? '' }, '-date'),
        enabled: !!consultationId,
    });
    ...
    const statusInfo = consultation?.status ? STATUS_CONFIG[consultation.status] : null;
