...
import type { AdminScope, DataClientBase, EntityRepository, RawEntities } from './contract';
...
export interface OpenReadEntity<T> extends EntityRead<T> {
  /** Acesso administrativo explícito, para clareza nos call sites de escrita. */
...
/** Acrescenta o acesso administrativo explícito a um repositório de leitura aberta. */
function withAdmin<T>(repo: EntityRead<T>): OpenReadEntity<T> {
  return { ...repo, asAdmin: () => repo };
}
...
export function createAppDataClient(
  ...
  return {
    ...
    entities: {
      ...
      Doctor: withAdmin(raw.Doctor as EntityRead<Doctor>),
      Template: withAdmin(raw.Template as EntityRead<Template>),
      AccessLog: withAdmin(raw.AccessLog as EntityRead<AccessLog>),
    },
  };
}
