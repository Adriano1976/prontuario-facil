...
/**
 * Relatórios de atendimento: volume mensal e ranking por especialidade.
 *
 * PARIDADE: comportamento e aparência idênticos ao anterior. Continuam iguais: a
 * janela de 12 meses, o filtro por agendamentos concluídos, o limite de 500 registros,
 * o agrupamento por mês, o ranking por especialidade com o rótulo "Não especificada"
 * quando o médico não tem especialidade, e os estados de carregamento e de lista vazia.
 *
 * PARIDADE DE LEITURA: as duas consultas passam a declarar o escopo de acesso, pelo
 * escopo do próprio usuário. É a mesma condição que a regra de acesso do servidor já
 * aplicava — nenhum resultado muda; o que muda é a omissão deixar de ser possível.
 */

/** Uma linha do volume mensal. */
...
/** Uma linha do ranking por especialidade. */
...
/**
 * Lê o escopo do próprio usuário, resolvido a partir da sessão.
 *
 * Reutilizável pelas consultas abaixo. Devolve escopo de usuário (nunca
 * administrativo), que é o que a leitura de dado clínico exige.
 */
async function escopoDaSessao() {
  const user = toSessionUser(await base44.auth.me());
  return asUserScope(user);
}
...
export default function ReportsView() {
  const { data: appointments, isLoading } = useQuery({
    ...
    queryFn: async () =>
      base44.entities.Appointment.filterOwned(
        await escopoDaSessao(),
        { status: 'concluido' },
        '-date',
        500,
      ),
  ...
  const specialtyData = useMemo<SpecialtyPoint[]>(() => {
            ...
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(value) => [`${String(value)} consultas`, 'Consultas']}
                />
          ...
          {specialtyData.length === 0 ? (
            ...
            <div className="space-y-3">
              {specialtyData.map((item, index) => {
                ...
                const overallPct =
                  totalConsultations > 0
                    ? Math.round((item.count / totalConsultations) * 100)
                    : 0;
                const isChampion = index === 0;
                return (
                  <div key={item.name} className="flex items-center gap-4">
                    <div
                      className={`flex items-center justify-center h-8 w-8 rounded-lg text-sm font-bold flex-shrink-0 ${
                        isChampion
