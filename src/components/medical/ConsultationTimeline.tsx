...
/**
 * Linha do tempo do histórico do paciente.
 *
 * PARIDADE: comportamento e aparência idênticos ao anterior. Continuam iguais: a
 * combinação das quatro origens de evento, a ordenação do mais recente para o mais
 * antigo, a prescrição datada pela data de criação, os estilos por tipo de evento, a
 * legenda de situação da consulta e do agendamento, e o estado vazio.
 *
 * Nota de correção de tipo (não de comportamento): a ordenação original subtraía
 * objetos de data diretamente. Em JavaScript isso funciona por conversão implícita; a
 * forma explícita usa os milissegundos, com resultado idêntico.
 */
...
/** Evento de consulta. */
type ConsultationEvent = Consultation & { eventType: 'consultation' };
/** Evento de documento clínico, datado pela criação. */
type PrescriptionEvent = Prescription & { eventType: 'prescription'; date: string };
/** Evento de exame. */
type ExamEvent = Exam & { eventType: 'exam' };
/** Evento de agendamento. */
type AppointmentEvent = Appointment & { eventType: 'appointment' };
...
export default function ConsultationTimeline({
  ...
  const events: TimelineEvent[] = [
    ...(consultations?.map((c): ConsultationEvent => ({ ...c, eventType: 'consultation' })) ||
      []),
    ...(prescriptions?.map(
      (p): PrescriptionEvent => ({ ...p, eventType: 'prescription', date: p.created_date }),
    ) || []),
    ...(exams?.map((e): ExamEvent => ({ ...e, eventType: 'exam' })) || []),
    ...(appointments?.map((a): AppointmentEvent => ({ ...a, eventType: 'appointment' })) || []),
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <Badge className={STATUS_CONFIG[event.status ?? 'agendada'].color}>
                        {STATUS_CONFIG[event.status ?? 'agendada'].label}
                      </Badge>
                    </div>
